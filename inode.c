                                ...
                                struct ps2fs_inode *ps2inode)
{
    ...
    {
        ps2fs_error(inode->i_sb, "ps2fs_read_raw_inode",
                    "unsupported block"
                    " size %lu",
...
bad_block:
    ...
    return -EIO;
bad_write:
    ps2fs_error(inode->i_sb, "ps2_write_raw_inode",
                "write of sector %lu"
                " failed",
                sector);
    brelse(bh);
...
void ps2fs_read_inode(struct inode *inode)
{
    ...
    if (inode->i_ino < sbinfo->root_inode ||
        ...
        PS2FS_BNUM(inode->i_ino) >= sbinfo->size[PS2FS_SUBPART(inode->i_ino)] >>
            sbinfo->block_shift)
    {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 4, 0)
void ps2fs_write_inode(struct inode *inode) { ps2fs_update_inode(inode, 0); }
#else
                    ...
                    struct buffer_head *bh_result, int create)
{
    ...
    { /* FIXME: 113 -> named constant */
        __u32 count = le16_to_cpu(ps2inode->data[i].count);
        dprintk("   %d: %08X/%u\n", i, le32_to_cpu(ps2inode->data[i].number),
                count);
        if (!count)
            break;
    ...
    if (create)
    {
        ...
        if (i == 113)
        {
            ...
            block =
                get_new_block(inode->i_sb, PS2FS_MAKE_INUM(nextblock, part));
        }
        else
        {
            /* Any block will do. */

                                     ...
                                     int silent)
{
    ...
    bh =
        bread(dev, sbinfo->first_sector[0] + (0x400000 / blocksize), blocksize);
    if (!bh)
    {
    ...
    {
        ps2fs_warning(sb, "ps2fs_read_super",
                      "root inode number (%d) too"
                      " small",
                      sbinfo->root_inode);
    }
        ...
        !sb->s_root->d_inode->i_blocks || !sb->s_root->d_inode->i_size)
    {
        if (sb->s_root)
        {
            ps2fs_error(sb, "ps2fs_read_super",
                        "root inode corrupt!"
...
static struct file_system_type ps2fs_fs_type = {"ps2fs", FS_REQUIRES_DEV,
                                                ps2fs_read_super, NULL};
...
module_init(init_ps2fs_fs) module_exit(exit_ps2fs_fs)

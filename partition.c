                   ...
                   __u32 size[PS2FS_MAX_SUBPARTS])
{
    ...
    do
    {
        ...
        {
            printk(
                "ps2fs: find_partition: (warning) bad checksum for"
        ...
        if (!id || strncmp(id, ph->id, PS2_PART_IDMAX) == 0)
        {
            ...
            for (i = 0; i < PS2_PART_MAXSUB && ph->subs[i].length; i++)
            {
                ...
                {
                    printk(
                        "ps2fs: find_partition: (warning) partition has"
                        " too many subparts (max %d)",
                        PS2FS_MAX_SUBPARTS);
                    return PS2FS_MAX_SUBPARTS;
                }
                start[i + 1] =
                    le32_to_cpu(ph->subs[i].start) / (blocksize / 512);
                size[i + 1] =
                    le32_to_cpu(ph->subs[i].length) / (blocksize / 512);
            }
            return i + 1;
        }
    } while ((block = next) != 0);
